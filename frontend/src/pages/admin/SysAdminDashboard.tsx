import { useNavigate } from 'react-router-dom';
import { Shield, LifeBuoy, FileText, Settings, Users, Activity } from 'lucide-react';

export function SysAdminDashboard() {
  const navigate = useNavigate();

  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">System Administration</h1>
          <p className="text-slate-500 mt-1">Manage global platform settings, services, and security</p>
        </div>
        <span className="px-4 py-2 bg-indigo-50 text-indigo-700 font-medium rounded-full text-sm">
          SysAdmin Role
        </span>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        
        <button
          onClick={() => navigate('/admin/audit')}
          className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all text-left flex flex-col group"
        >
          <div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center mb-4 group-hover:bg-slate-900 group-hover:text-white transition-colors">
            <Shield size={24} />
          </div>
          <h3 className="text-lg font-bold text-slate-900 mb-2">Audit Logs</h3>
          <p className="text-slate-500 text-sm">
            View system-wide security logs, administrative actions, and access trails.
          </p>
        </button>

        <button
          onClick={() => navigate('/admin/support')}
          className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-indigo-300 transition-all text-left flex flex-col group"
        >
          <div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center mb-4 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
            <LifeBuoy size={24} />
          </div>
          <h3 className="text-lg font-bold text-slate-900 mb-2">Support Tickets</h3>
          <p className="text-slate-500 text-sm">
            Manage citizen support queries, view all tickets, and provide assistance.
          </p>
        </button>

        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 opacity-75">
          <div className="w-12 h-12 bg-slate-200 text-slate-500 rounded-xl flex items-center justify-center mb-4">
            <FileText size={24} />
          </div>
          <h3 className="text-lg font-bold text-slate-900 mb-2 flex items-center gap-2">
            Service Catalogue
            <span className="text-xs bg-slate-200 px-2 py-0.5 rounded-full text-slate-600">Soon</span>
          </h3>
          <p className="text-slate-500 text-sm">
            Configure new services, update fee structures, and manage required documents.
          </p>
        </div>

        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 opacity-75">
          <div className="w-12 h-12 bg-slate-200 text-slate-500 rounded-xl flex items-center justify-center mb-4">
            <Users size={24} />
          </div>
          <h3 className="text-lg font-bold text-slate-900 mb-2 flex items-center gap-2">
            Centre & Staff
            <span className="text-xs bg-slate-200 px-2 py-0.5 rounded-full text-slate-600">Soon</span>
          </h3>
          <p className="text-slate-500 text-sm">
            Register new Akshaya Centres and onboard Centre Administrators.
          </p>
        </div>

      </div>
    </div>
  );
}
